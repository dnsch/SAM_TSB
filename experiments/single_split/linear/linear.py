...
class LinearStandardExperiment(TorchSingleSplitExperiment):
    ...
    def _run_hessian_analysis(
        ...
        """Custom Hessian analysis for Linear model."""
        from third_party.utils.pyhessian.pyhessian import hessian

        logger.info("Computing Hessian analysis...")

        # Wrapper for Linear model to handle dimension transformation
        class LinearHessianWrapper(torch.nn.Module):
            ...
            def forward(self, x):
                ...
                # Linear model expects: [batch, seq_len, channels]
                x = x.permute(0, 2, 1)
                out = self.linear_model(x)
                # Output: [batch, pred_len, channels]
                # Return as [batch, channels, pred_len] to match target
                return out.permute(0, 2, 1)

        wrapped_model = LinearHessianWrapper(model)

        # pyhessian expects cuda to be a boolean, not a device string
        ...
        hessian_comp = hessian(
            ...
            cuda=use_cuda,  # Pass boolean, not device string
        )
...
if __name__ == "__main__":
    run_single_split_experiment(LinearStandardExperiment)
