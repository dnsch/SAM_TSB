...
class TorchEngine(ABC):
    def __init__(
        ...
        self._revin_subtract_last = revin_subtract_last
        self._revin = None

        if self._use_revin:
            self._revin = RevIN(
                num_features=self.model.input_channels,
                affine=self._revin_affine,
                subtract_last=self._revin_subtract_last,
            ).to(self._device)
        ...
        self._training_stats_file = self._stats_path / "training_statistics.csv"
    ...
    def _has_time_features(self, batch):
        """
    ...
    def _compute_test_metrics(self, preds: torch.Tensor, labels: torch.Tensor) -> Dict[str, float]:
        ...
        # Save evaluation CSV
        total_train_time = getattr(self, "_total_train_time", 0.0)
        mean_epoch_time = getattr(self, "_mean_epoch_time", 0.0)
        self._save_evaluation_csv(avg_metrics, total_train_time, mean_epoch_time)
    ...
    def _init_training_stats_csv(self):
        """Initialize the training statistics CSV file with header."""
        with open(self._training_stats_file, "w") as f:
            f.write("epoch,training_loss,validation_loss,learning_rate,train_time\n")

    def _append_training_stats(
        self, epoch: int, train_loss: float, val_loss: float, lr: float, train_time: float
    ):
        """Append a row to the training statistics CSV."""
        with open(self._training_stats_file, "a") as f:
            f.write(f"{epoch},{train_loss},{val_loss},{lr},{train_time}\n")
    ...
    def _save_evaluation_csv(
        ...
        with open(eval_file, "w") as f:
            ...
            # Write values
            num_params = self.model.param_num()
            values = list(avg_metrics.values()) + [total_train_time, mean_epoch_time, num_params]
    ...
    # TODO: have updated function that creates better plots.
    # remove this one
    ...
    def _optimizer_step(self, loss: torch.Tensor, x_batch, y_batch):
        ...
        Args:
            ...
            x_batch: Input batch (used for SAM/GSAM/FSAM)
            y_batch: Target batch (used for SAM/GSAM/FSAM)
        """

        if self._sam or self._fsam:
            # SAM/FSAM optimizer
            loss.backward()
            self._optimizer.first_step(zero_grad=True)

            # Second forward pass - apply RevIN again
            x_batch_norm = self._revin_norm(x_batch)
            # TODO: maybe remove flatten output
            out_batch = self._forward_pass(x_batch_norm).contiguous()
            out_batch = self._revin_denorm(out_batch)

            prepared_out_batch = self._prepare_predictions(out_batch)
            prepared_y_batch = self._prepare_ground_truths(y_batch)
            loss = self._compute_loss(prepared_out_batch, prepared_y_batch)
            ...
            # GSAM with custom closure for RevIN support
            prepared_y_batch = self._prepare_ground_truths(y_batch)

            def gsam_closure():
                self._optimizer.base_optimizer.zero_grad()
                with torch.enable_grad():
                    x_norm = self._revin_norm(x_batch)
                    outputs = self._forward_pass(x_norm)
                    outputs_denorm = self._revin_denorm(outputs)
                    outputs_denorm = self._prepare_predictions(outputs_denorm)
                    loss = self._compute_loss(outputs_denorm, prepared_y_batch)
                ...
                return outputs_denorm, loss_value

            predictions, loss = self._optimizer.step(closure=gsam_closure)
    ...
    def _get_current_lr(self):
        ...
        # No scheduler - return constant LR
        if self._lr_scheduler is None:
            if hasattr(self._optimizer, "param_groups"):
    ...
    # TODO: change name
    def _should_save_epoch_model(self, epoch: int):
        """
    ...
    def _check_timeout(self, start_time: float, epoch: int):
        """
    ...
    def train(self) -> Optional[float]:
        ...
        for epoch in range(self._max_epochs):
            ...
            loss_name = self._get_loss_name()
            train_loss = train_metrics.get(loss_name, float("nan"))
            val_loss = val_metrics.get(loss_name, float("nan"))
            self._append_training_stats(epoch + 1, train_loss, val_loss, current_lr, train_time)
        ...
        self._total_train_time = time.time() - start_time
        self._mean_epoch_time = np.mean(epoch_times) if epoch_times else 0.0
