...
class TSMixer_Engine(TorchEngine):
    ...
    def _prepare_test_data(self, preds, labels) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        preds = preds.reshape(preds.shape[0], self.model.input_channels, self.model.pred_len)
        labels = labels.reshape(labels.shape[0], self.model.input_channels, self.model.pred_len)
        return preds, labels
