...
class FSAM(torch.optim.Optimizer):
    def __init__(
        self, params, base_optimizer, rho=0.05, sigma=1, lmbda=0.9, adaptive=False, **kwargs
    ):
        ...
        self.sigma = sigma
        self.lmbda = lmbda
        print("FSAM sigma:", self.sigma, "lambda:", self.lmbda)
        ...
        for group in self.param_groups:
            for p in group["params"]:
                ...
                if not "momentum" in self.state[p]:
                    ...
                    self.state[p]["momentum"] = self.state[p]["momentum"] * self.lmbda + grad * (
                        1 - self.lmbda
                    )
