...
from src.utils.plotting import plot_dataset_splits
...
from src.utils.paths import get_autoformer_dataset_path
...
def extract_time_features_raw(dates: pd.DatetimeIndex, freq="h") -> np.ndarray:
    """
...
def extract_time_features(dates: pd.DatetimeIndex, freq="h", timeenc=1) -> np.ndarray:
    """
...
def construct_sliding_window_data(
    ...
    """
    # TODO: add check, if n_samples negative, stop with error message (means
    # that the sequence is larger than available data)
...
class TSMixerExtDataset(Dataset):
    ...
    def __init__(
        ...
        Args:
            ...
            y: Target data (n_samples, num_channels, pred_len)
            x_mark: Time features for historical period (n_samples, seq_len, num_time_features)
            y_mark: Time features for prediction period (n_samples, pred_len, num_time_features)
        ...
        if y_mark is not None:
            ...
            # Create placeholder zeros: (n_samples, pred_len, 1)
            pred_len = y.shape[1] if y.ndim == 2 else y.shape[2]
            self.x_extra_future = torch.zeros(n_samples, pred_len, 1)
    ...
    def __getitem__(self, idx):
        ...
        return (
            ...
            self.x_extra_future[idx],  # (pred_len, extra_channels)
            self.x_static[idx],  # (static_channels,)
            y,  # (pred_len, num_channels)
        )
...
class AutoformerDataloader:
    def __init__(
        ...
        # file_path = (
        #     SCRIPT_DIR.parents[2] / "data" / "autoformer_datasets" / f"{dataset}.csv"
        # )
        file_path = get_autoformer_dataset_path(dataset=dataset)
        ...
        # Add shuffling of train and validation data, some train samples go into
        # validation set, some val samples go into train
        # to reduce possible distribution shift issues
        self.shuffle_train_val = shuffle_train_val

        # Plot the used dataset
        ...
        self._input_channels = df_raw.shape[1]
    ...
    def get_input_channels(self, override: Optional[int] = None):
        """
        Get the number of input channels, either from args, or derived from
        dataloader.
        """
    ...
    # needed for tsmixer_ext
    def get_extra_channels(self, override: Optional[int] = None):
        """
        Get the number of extra channels, either from args, or derived from
        dataloader.
        """
        if override is not None:
            return override

        return self.dataloader["train_loader"].dataset[0][1].shape[-1]

    # needed for tsmixer_ext
    def get_static_channels(self, override: Optional[int] = None):
        """
        Get the number of static channels, either from args, or derived from
        dataloader.
        """
        if override is not None:
            return override
        return self.dataloader["train_loader"].dataset[0][3].shape[0]
    ...
    def _shuffle_train_val_samples(
        self,
        x_train,
        y_train,
        x_val,
        ...
        """
        Combine train and val samples, shuffle them, and re-split.
        This reduces distribution shift between train and validation sets.
        """
        ...
        # Handle time marks if present
        has_marks = x_train_mark is not None
        if has_marks:
            x_mark_combined = np.concatenate([x_train_mark, x_val_mark], axis=0)
            y_mark_combined = np.concatenate([y_train_mark, y_val_mark], axis=0)

        # Shuffle with seed for reproducibility
        n_combined = len(x_combined)
        rng = np.random.default_rng(self.seed)
        shuffle_idx = rng.permutation(n_combined)

        x_combined = x_combined[shuffle_idx]
        y_combined = y_combined[shuffle_idx]
        if has_marks:
            x_mark_combined = x_mark_combined[shuffle_idx]
            y_mark_combined = y_mark_combined[shuffle_idx]

        # Re-split with original proportions
        x_train_new = x_combined[:n_train_original]
        y_train_new = y_combined[:n_train_original]
        x_val_new = x_combined[n_train_original:]
        y_val_new = y_combined[n_train_original:]

        if has_marks:
            x_train_mark_new = x_mark_combined[:n_train_original]
            y_train_mark_new = y_mark_combined[:n_train_original]
            x_val_mark_new = x_mark_combined[n_train_original:]
            y_val_mark_new = y_mark_combined[n_train_original:]
            return (
                x_train_new,
                y_train_new,
                x_val_new,
        ...
        return x_train_new, y_train_new, x_val_new, y_val_new, None, None, None, None
        ...
        if multi_split:
            ...
            if self.shuffle_train_val:
                (
                    ...
                    x_val_mark,
                    y_val_mark,
                ) = self._shuffle_train_val_samples(
                    x_train,
                    y_train,
                    x_val,
                    y_val,
...
class StatsforecastDataloader:
    def __init__(
        ...
        # Use same split logic as AutoformerDataloader
        if dataset.startswith("ETTm"):
            train_end = 12 * 30 * 24 * 4
            val_end = train_end + 4 * 30 * 24 * 4
            test_end = val_end + 4 * 30 * 24 * 4
        ...
        if merge_train_val:
            ...
            #
            # This approach is handled by the multi_split parameter:
        ...
        # But let's keep this functionality
            ...
            # Create dataloader dictionary (similar to AutoformerDataloader)
            self.dataloader = {
