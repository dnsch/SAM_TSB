...
def plot_samformer_attention_mean(
    ...
    attention_patterns_mean = attention_patterns_per_epoch[-2].mean(dim=0).detach().cpu().numpy()

    # Limit display size for readability
    if attention_patterns_mean.shape[0] > max_display_size:
        attention_patterns_mean = attention_patterns_mean[:max_display_size, :max_display_size]
...
def plot_samformer_attention_mean_stats(attention_patterns_per_epoch, epoch, save_path):
    ...
    attention_patterns_mean = (
        torch.cat(attention_patterns_per_epoch, dim=0).mean(dim=0).detach().cpu().numpy()
    )
    ...
    epsilon = 1e-10
    entropy = -np.sum(attention_patterns_mean * np.log(attention_patterns_mean + epsilon), axis=1)
...
def plot_samformer_attention_variance(
    ...
    if attention_patterns_var.shape[0] > max_display_size:
        attention_patterns_var = attention_patterns_var[:max_display_size, :max_display_size]
...
def get_nixtla_model(args):
    ...
    if model_name_lower == "autoarima":
        ...
        seasonal_periods = _get_param_with_warning(args, "season_length", 24, "seasonal_periods")
        ...
        prediction_intervals = _get_param_with_warning(args, "prediction_intervals", None)
        ...
        raise ValueError(f"Unknown model: '{args.model_name}'. Supported models: 'arima', 'mfles'")
