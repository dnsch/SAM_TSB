...
SCRIPT_DIR = Path(__file__).resolve().parent.parents[1]
...
def plot_2d_contour(
    ...
    plot_dir = Path(surf_file)
    plot_dir = plot_dir.parents[1] / "plots" / "loss_surface"
    plot_dir.mkdir(parents=True, exist_ok=True)
    # # --------------------------------------------------------------------
    # # Plot 2D contours
    # # --------------------------------------------------------------------
    # #
    # # Define desired directory and make sure it exists
    #
    # plot_dir = Path(surf_file)
    # plot_dir = plot_dir.parents[1] / "plots" / "loss_surface"
    # plot_dir.mkdir(parents=True, exist_ok=True)
    ...
    # file_name = f"{Path(surf_file).name}_{surf_name}_2dcontourf.png"
    ...
    file_name = f"{Path(surf_file).name}_{surf_name}_2dheat.png"
    save_path = plot_dir / file_name
    ...
    # ax = Axes3D(fig)
    Z = np.clip(Z, a_min=Z.min(), a_max=vmax)
    surf = ax.plot_surface(X, Y, Z, cmap=cm.coolwarm, linewidth=0, antialiased=False)
    fig.colorbar(surf, shrink=0.5, aspect=5)
    file_name = f"{Path(surf_file).name}_{surf_name}_3dsurface.png"
    save_path = plot_dir / file_name
    fig.savefig(
        save_path,
        dpi=300,
...
def plot_trajectory(proj_file, dir_file, show=False):
    ...
    fig = plt.figure()
    x_coord = f["proj_xcoord"][()]
    y_coord = f["proj_ycoord"][()]
...
def plot_contour_trajectory(
    ...
    x_coord = pf["proj_xcoord"][()]
    y_coord = pf["proj_ycoord"][()]
    ...
    file_name = proj_file + "_" + surf_name + "_2dcontour_proj.png"
    save_path = plot_dir / file_name
    fig.savefig(
        save_path,
        dpi=300,
...
def plot_2d_eig_ratio(surf_file, val_1="min_eig", val_2="max_eig", show=False):
    ...
    sns_plot.invert_yaxis()
    file_name = (
        Path(surf_file).name + "_" + val_1 + "_" + val_2 + "_abs_ratio_heat_sns.png"
    )
    save_path = plot_dir / file_name
    sns_plot.get_figure().savefig(
        save_path,
    ...
    sns_plot.invert_yaxis()
    file_name = Path(surf_file).name + "_" + val_1 + "_" + val_2 + "_ratio_heat_sns.png"
    save_path = plot_dir / file_name
    sns_plot.get_figure().savefig(
        save_path,
        dpi=300,
